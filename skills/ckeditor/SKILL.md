...
as markdown: swap `.html` → `.md`), `llms-full.txt`, the npm-shipped TypeScript
types, and `llms.txt` — each with a different sweet spot, **not ranked**. Pick by task, and **route big fetches through a sub-agent**
